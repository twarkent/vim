...
syn keyword verilogStatement   always and assert assign automatic buf always_comb always_latch always_ff
syn keyword verilogStatement   package endpackage constraint
syn keyword verilogStatement   bit bufif0 bufif1 cell clocking cmos
syn keyword verilogStatement   chandle class config covergroup deassign defparam design
syn keyword verilogStatement   disable do edge endclass endclocking endconfig endcover
syn keyword verilogStatement   endfunction endgenerate endinterface endmodule
syn keyword verilogStatement   endprimitive endspecify endtable endtask
syn keyword verilogStatement   event final force foreach function
syn keyword verilogStatement   generate genvar highz0 highz1 ifnone
syn keyword verilogStatement   import incdir include initial inout input
syn keyword verilogStatement   instance int integer interface large liblist
syn keyword verilogStatement   library local localparam logic macromodule medium
syn keyword verilogStatement   modport module nand negedge nmos nor
syn keyword verilogStatement   noshowcancelled not notif0 notif1 or
syn keyword verilogStatement   output packed parameter pmos posedge primitive
...
syn keyword verilogLabel       begin end fork join join_any join_none
...
syn keyword verilogNote contained NOTE
...
"syn region  verilogComment start="/\*" end="\*/" contains=verilogNote,@Spell
"syn match   verilogComment "//.*" contains=verilogNote,@Spell
...
syn match verilogGlobal "timeunit"
syn match verilogGlobal "timeprecision"
